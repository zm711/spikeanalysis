...
class IntrinsicPlotter(PlotterBase):
    ...
    def plot_acs(self, sp: Union[SpikeData, SpikeAnalysis], ref_dur_ms: float = 2.0):
        ...
        for cluster in cluster_ids:
            ...
            ax.set(xlim=(np.min(bin_centers_vals), np.max(bin_centers_vals)), xlabel=self.x_axis, ylabel='Spike Counts')         # refract lines
    ...
    def plot_pcs(self, sp: Optional[SpikeData]=None):
        ...
        if sp is None:
            ...
            for cluster in cluster_ids:
                ...
                other_spikes_included = (
                    (sparse_pcs[:, top_chans[0]]!=0)==(sparse_pcs[:, top_chans[1]]!=0)
                ) == (cluster_ids!=cluster)
    ...
    def _sparse_pcs(self, pc_feat, pc_feat_ind, templates, n_per_chan, n_pc_chans):
        ...
        for this_feat in range(n_per_chan):
            col_inds[this_feat*nspikes*n_pc_chans : (this_feat+1)*nspikes*n_pc_chans] = (col_ind_temp * n_per_chan + this_feat)

        pc_feat_rs = np.zeros((nspikes*n_pc_chans*n_per_chan))

        for this_feat in range(n_per_chan):
            pc_feat_rs[this_feat * nspikes*n_pc_chans: (this_feat+1)*nspikes*n_pc_chans] = np.reshape(np.squeeze(pc_feat[:, this_feat, :]), nspikes*n_pc_chans, order="F")
