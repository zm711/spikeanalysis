...
class MergedSpikeAnalysis(SpikeAnalysis):
    ...
    def merge_data(self):
        ...
        for stim in self.events.values():
            for sa in self.spikeanalysis_list:
                for event in sa.events:
                    if self._get_stim_key(stim["stim"]) is None:
                        raise RuntimeError("Can not combine incompatible data")

                for sampling_rate in sa._sampling_rate:
        ...
        sub_cluster_ids = np.concatenate(
            [
                str(self.name_list[idx]) + str(spike)
        ...
        for stim_name, stim_key in event_names.items():
            tg_0 = set(self.events[0][stim_key]["trial_group"])
            tg_0_len = len(tg_0)
            # start with easy case of checking if number of trial groups the same
            if all([len(set(x[self._get_stim_key(stim_name)]["trial_groups"])) == tg_0_len for x in self.events_list]):
                # if same number now we need to confirm same values
                for event in self.events_list:
                    for tg in tg_0:
                        if tg not in event[self._get_stim_key(stim_name)]["trial_groups"]:
                            unbalanced = True
                            break
                        else:
                            unbalanced = False
    ...
    def _get_stim_key(event, stim_name):

        for key, value in event.items():
            if value["stim"] == stim_name:
                return key

    def _fill_merged_data(self, stim, fill, data_list):

        if not np.isnan(fill) or type(fill, (int, float)):
            raise TypeError("fill should be nan or ideally 0")

        tg_list = [set(x[self._get_stim_key(stim)] for x in self.events_list)]
    ...
    def get_raw_firing_rate(
        ...
        for stim in firing_rates_list[0].keys():
            if self._matched_trial_groups[stim]:
                merged_firing_rates[stim] = np.concatenate(firing_rates_list, axis=0)
            else:
                self._fill_merged_data(stim, fill, firing_rates_list)
                merged_firing_rates[stim] = np.concatenate(firing_rates_list, axis=0)

        self.mean_firing_rate = merged_firing_rates
        self.fr_bins = self.spikeanalysis_list[0].fr_bins

    def z_score_data(self, time_bin_ms, bsl_window, z_window, eps, fill):

        z_score_list = []
        for sa in self.spikeanalysis_list:
            sa.z_score_data(time_bin_ms, bsl_window, z_window, eps)
            z_score_list.append(sa.return_value("zscores"))

        merged_z_scores = {}
        for stim in z_score_list[0].keys():
            if self._matched_trial_groups[stim]:
                merged_z_scores[stim] = np.concatenate(z_score_list, axis=0)
            else:
                self._fill_merged_data(stim, fill, z_score_list)
                merged_z_scores[stim] = np.concatenate(z_score_list, axis=0)
    ...
    def get_interspike_intervals(self):
        return super().get_interspike_intervals()
